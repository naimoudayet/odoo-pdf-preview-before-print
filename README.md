...
The module registers a handler in Odoo 16's `ir.actions.report handlers` registry. When any `qweb-pdf` report action is triggered, the handler intercepts it and opens a `PreviewDialog` component with the PDF rendered in an iframe. The user then chooses to print, download, or close.
